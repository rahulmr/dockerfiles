...
# Set Default CHARACTERSET as AL32UTF8
export ORACLE_CHARACTERSET=${4:-AL32UTF8}

# Set Default domain as local
export ORACLE_DOMAIN=${5:-local}
...
sed -i -e "s|###ORACLE_DOMAIN###|$ORACLE_DOMAIN|g" $ORACLE_BASE/dbca.rsp
...
(DESCRIPTION = 
  ...
  (CONNECT_DATA =
    (SERVER = DEDICATED)
    (SERVICE_NAME = $ORACLE_PDB.$ORACLE_DOMAIN)
  )
